...
func RunOperator(ctx context.Context, controllerConfig *controllercmd.ControllerContext, guestKubeConfigFile string) error {
	...
	gvk := operatorv1.SchemeGroupVersion.WithKind("CSISnapshotControllers")
	guestOperatorClient, dynamicInformers, err := goc.NewClusterScopedOperatorClientWithConfigName(clock.RealClock{}, guestKubeConfig, gvr, gvk, "cluster", extractApplySpec, extractApplyStatus)
	if err != nil {
		return err
	}
